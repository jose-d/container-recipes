...
c.LDAPAuthenticator.server_address = 'ldap.fzu.cz'

c.LDAPAuthenticator.lookup_dn = False
c.LDAPAuthenticator.bind_dn_template = [
  "uid={username},ou=people,dc=fzu,dc=cz",
]
...
c.JupyterHub.hub_ip = '172.16.120.197' # liste
c.SlurmSpawner.hub_connect_ip = '172.16.120.197' # or whatever the Hub's IP is, as seen by the Spawners
...
{{prologue}}
mkdir -p "{{homedir}}/.jh/koios_home"
which jupyterhub-singleuser
{% if srun %}{{srun}} {% endif %}singularity exec --pwd={{homedir}} --hostname=python368-container --home={{homedir}}/.jh:{{homedir}} --bind={{homedir}}:{{homedir}}/koios_home /home/soft/containers/jupyter/jupyterhub-singularity.sif {{cmd}}
...
c.Authenticator.admin_users = ['jose',]
